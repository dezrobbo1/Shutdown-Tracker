...
This record is retained as technical history. Its exact candidate, approval, and manual round-trip requirements are not active product authority. The immutable-source, MSPDI/XML, no-native-`.mpp`, no-independent-scheduler, and no-silent-master-write principles remain active through ADR-012.

## Context

Shutdown Tracker imports Microsoft Project schedule snapshots, captures live execution information, allows authorised Tier 1 input in the Master Console, and must return reviewed information to Tier 1 for application review and to the relevant schedule owner or Microsoft Project operator for external schedule handling without becoming a second scheduling engine or silently changing the accepted master schedule.
...
- Allow Tier 1 to reject the candidate, retain it for further review, or record an adopt/merge disposition, with the relevant schedule owner or Microsoft Project operator performing the external Project action.
...
“Schedule owner” and “Microsoft Project operator” describe external schedule-handling activity. They do not create additional Shutdown Tracker application roles, and not every Tier 1 user is assumed to perform that activity.
...
- Candidate review must distinguish approved Shutdown Tracker inputs, Project-calculated consequences, manual schedule-owner or Microsoft Project operator edits made in Project, and unexplained differences.
...
- Native `.mpp` generation by Shutdown Tracker remains out of scope; the relevant schedule owner or Microsoft Project operator may nevertheless save a reviewed candidate as a Project file using Microsoft Project itself after the required Tier 1 review/disposition.
