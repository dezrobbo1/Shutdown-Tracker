...
Audit events provide a durable record of security, membership, assignment, operational, import, experimental export, offline-sync, discussion, evidence, and reporting activity. They support accountability, incident review, provenance, evidence handling, and compliance reporting.

Export/candidate fields and event names in the applied schema or existing runtime are compatibility vocabulary. Under [ADR-012](../adr/ADR-012-product-trial-foundation-and-export-deferral.md), they do not define the active export product contract.
...
### Task progress and experimental Project-input compatibility
...
The `export_candidate_*` names describe retained technical compatibility only. A future Project-input decision model requires a new approved contract; these names do not require exact candidate approval in the active product trial.
...
### Experimental export compatibility and offline sync
...
- Mobile execution events, field progress observations, operational review, and Critical reporting remain separate events.
- Any future Project-input/export workflow must introduce explicit event semantics rather than treating ordinary progress as implicit approval.
- Local capture time and server-received time are both preserved when offline updates sync later.
- Re-import conflicts remain visible when imported facts conflict with active Tracker execution history.
...
- Existing Project-input/candidate audit rows remain historical technical records; a replacement export contract must define its own provenance requirements.
