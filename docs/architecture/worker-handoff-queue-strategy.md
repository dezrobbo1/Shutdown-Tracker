...
Status: import-processing direction plus retained experimental export compatibility.

Shutdown Tracker uses the API service for workflow ownership and the project worker for Microsoft Project file processing. The queue/background-job layer should make approved processing reliable without turning the API into a parser or turning the worker into the product workflow authority.

[ADR-012](../adr/ADR-012-product-trial-foundation-and-export-deferral.md) defers the final Project export contract. Export-job material below documents existing technical boundaries only; it is not an active implementation sequence or product requirement.

## Current Baseline

The repository already has two synchronous, opt-in local paths:
...
- Experimental export artifact generation: `POST /api/projects/{projectId}/export-preview/{exportBatchId}/generate-artifact` calls the worker endpoint `POST /worker/project-export/generate-artifact`.
...
The active strategy covers future asynchronous processing for:

- Worker-owned MPXJ import parsing from an already stored source file.

It also preserves design notes for worker-owned MSPDI/XML generation from an existing export batch. That path remains experimental and must not be productised without a replacement export ADR.
...
- Existing export-batch lifecycle only as experimental compatibility state.
...
- Creating MSPDI/XML export artifacts under the retained experimental contract.
...
| Experimental export batches | `draft_preview`, `awaiting_approval`, `approved`, `rejected`, `generated`, `opened_in_microsoft_project`, `verified`, `superseded`, `failed` (compatibility vocabulary, not current product states) |
...
If the experimental export path is maintained, its export batch remains `approved` while a background worker job is queued or running. It reaches `generated` only after the API records the returned artifact URI/hash, or `failed` after terminal generation failure. This rule protects existing technical state; it does not approve the workflow.

## Job Types and Experimental Compatibility
...
| `project_export_artifact_generation` | Existing experimental export generation is requested. | Retained compatibility checks pass. | Generate the technical MSPDI/XML artifact at the API-reserved storage target and return URI/hash/summary. | Verify returned URI matches reserved target and record the compatibility result; no product readiness or master update is implied. |
...
Experimental export artifact jobs currently reference:

- `project_id`
- `export_batch_id`
- `project_snapshot_id`
- legacy leaf-task update records required by the compatibility contract
...
Audit events should record user-visible decisions and accepted workflow requests, such as a parse request accepted by the API. Existing export-generation, artifact, Project-open, and verification events remain compatibility history only and must not be presented as an active acceptance gate or master-schedule update.
...
A future queue provider should be selected only after an approved import-processing need or a separately approved future export contract is stable enough to justify implementation. Evaluate:
...
- Clear migration path from the current synchronous HTTP processing boundary.
...
1. Keep current synchronous import HTTP processing as the local/review baseline; leave experimental export processing unchanged.
...
4. Add an import worker consumer that reuses the existing import-processing service.
5. Add completion handling that updates existing import workflow records through the API-owned services.
6. Add dead-letter/failure handling that moves product records to existing `failed` states only when failure is terminal.

Each implementation PR should include tests proving no API-side Project parsing, no duplicate product records on retry, no unjustified product enum values, no Project write-back, and no schedule calculation. Export queue work requires a new product decision and ADR before implementation.
