...
Tier 1 authority is not narrowed by a category, filter, saved view, discipline, contractor, WBS branch, Resource `Group`, or Critical membership. It remains subject to project lifecycle, append-only audit, evidence controls, and the exact Microsoft Project candidate-approval boundary.
...
6. enforce the requested operation's record-state, audit, evidence, and Microsoft Project handoff rules.
