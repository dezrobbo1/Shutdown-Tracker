...
This is a **Tracker field progress observation**. Ordinary field users are not shown Microsoft Project-specific `% Work Complete` or `Physical % Complete` terminology. Interpreting a reviewed Tracker observation as a permitted Project input is a separate handoff-policy concern and is not changed by this model.
