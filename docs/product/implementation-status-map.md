...
| Production three-tier membership | Designed, not built | Static review personas and the browser-local Tier 1 round-trip identity are not production membership, entitlement, identity, or authorization records. |
...
| Today | Verified in repository | The Tier 1 round-trip trial derives a bounded 24-hour projection and attention state from a temporary imported XML schedule. Ordinary Today remains static; there is no production query/API. |
| Tasks explorer | Verified in repository | The Tier 1 round-trip trial browses and searches the imported XML hierarchy. The ordinary explorer remains static; there is no production project-task query, schedule editor, or recalculation. |
| Task Dashboard | Verified in repository | The Tier 1 round-trip trial derives imported task context, local execution, progress, problems/actions, and history from browser-memory state. The ordinary dashboard remains static and there is no production record API. |
...
| Approved Critical UI | Static visual only | Console Critical statically represents selected items/work packs, ownership, policy, obligations, reports, and history. Imported Project Critical values may appear as read-only round-trip context. No configuration/submission runtime or production Critical API exists. |
...
| Ordinary Console Export | Static visual only | Import / Export marks export as not finalised. The explicit round-trip trial is a separate evidence mode and must not make main's compatibility writer or PR #48's candidate pipeline the required product workflow. |
