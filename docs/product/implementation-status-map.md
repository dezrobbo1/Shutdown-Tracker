...
| Today | Verified in repository | The Tier 1 round-trip trial derives a bounded 24-hour projection and attention state from a temporary imported XML schedule. Ordinary Today shows no active-project state; there is no production query/API. |
| Tasks explorer | Verified in repository | The Tier 1 round-trip trial browses and searches the imported XML hierarchy. Ordinary mode shows no active-project state; there is no production project-task query, schedule editor, or recalculation. |
| Task Dashboard | Verified in repository | The Tier 1 round-trip trial derives imported task context, local execution, progress, problems/actions, and history from browser-memory state. Ordinary mode has no fabricated task dashboard and there is no production record API. |
| Task execution | Verified in repository | The Tier 1 round-trip trial implements Tier 1 Can't Start, Start, Pause, Resume, Finish, controlled trial timestamps, late-start context, pause/problem separation, and field-progress observations for imported executable leaves. There is no persisted execution API, audited correction path, production authorization, or offline behaviour. |
...
| Tier 1 browser-local Project round-trip product trial | Verified in repository — frontend/local Tier 1 round-trip product trial | With `VITE_SHUTDOWN_TRACKER_TIER1_ROUNDTRIP_TRIAL=true`, retained original UTF-8 bytes and losslessly decoded XML/MSPDI text can drive a temporary imported hierarchy, Tier 1 leaf execution, optional reviewed Actual Start/Finish or `% Complete`/`Physical % Complete` mappings, and a separate complete-source candidate. The UI guides an external manual Project recalculation and accepts a result XML for conservative local comparison; the repository does not perform or verify that external recalculation. It is browser-only XML/MSPDI evidence with no production persistence, native `.mpp`, final export contract, backend candidate workflow, automatic Project operation, or Project recalculation inside Shutdown Tracker. |
...
- The absence of PR #48's candidate pipeline and RoundTripWorkspace is deliberate; no mandatory manual Microsoft Project acceptance gate is part of the production product path. The separately flagged round-trip trial includes an evidence-only manual Project step.
