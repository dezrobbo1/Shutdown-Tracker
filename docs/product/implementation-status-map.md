...
| Mobile Assigned Tasks / Task Detail | Static visual only | `apps/mobile-pwa/src/App.tsx` represents the assigned-task-only shell, Tier 2 tracking, Tier 3 `WORKING_ON`/`FIELD_CONTROL`, system-timestamp action copy, end-of-shift field progress, task-owned operational sections, contextual Tier 2 Critical reporting, and compact sync state with synthetic data. |
...
| Task execution | Static visual only | Console/Mobile show disabled Can't Start, Start, Pause, Resume, and Finish controls plus automatic-time, late-start, pause/delay, resume/open-problem, finish-confirmation, and end-of-shift semantics. `task_execution_state` vocabulary exists, but there is no event/timestamp persistence, API, audited correction workflow, policy enforcement, or event-derived projection. |
...
| Task Dashboard | Static visual only | The Console represents Overview, Execution, People, Discussion, Delays / Problems, Actions, Evidence, History, Project/import/export context, and the approved five action labels/system-time boundary with synthetic data and disabled writes; no aggregate or execution API exists. |
...
| Retained Critical schema foundation | Verified in repository | V006 contains policy versions/effective periods, cadence and required-field JSON, reporting periods bound to a policy version, immutable updates, and supersession. V001/V006 retain shift/event/custom compatibility terms. This does not authorise arbitrary form construction or prove the approved workflow. |
| Approved Critical schema/model | Designed, not built | Selected-leaf source, explicit Tier 2 reporting owner, reusable template identity/isolated override behaviour, supported-catalogue enforcement, policy composition, and execution-fact reuse/deduplication are not represented end to end. Future schema changes must be additive; V006 remains unchanged. |
...
| Approved Critical UI shell | Static visual only | Console Critical represents selected leaves/work packs, Tier 2 owner, template/policy version, interval/time/shift/request/event mechanisms, supported required content, due/latest/condition/history state, and disabled configuration. Mobile shows known facts and remaining Tier 2 judgement fields in Task Detail. There is no policy evaluator, API, persistence, or production write. |
