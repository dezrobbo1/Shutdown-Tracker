...
| Project list / Projects Home | Designed, not built | The Console mounts the ordinary review shell or guarded round-trip workspace from `apps/console/src/main.tsx`; there is no production project list, search, lifecycle grouping, or switcher. |
...
| Project lifecycle | Designed, not built | `infra/migrations/V002__projects_snapshots_and_imports.sql` has partial project status storage but no approved Draft/Active/Closed/Archived lifecycle service or UI. |
...
| Tier 2-to-Tier 3 direct-report relationships | Designed, not built | No direct-report persistence, API, authorization check, or application surface exists. Target audit vocabulary does not implement the relationship. |
...
| Approved Critical UI | Designed, not built | Neither client implements the approved Console Critical configuration/oversight surface or assigned-task Tier 2 reporting workflow. |
...
| Import review | Verified in repository | `ImportReviewController` and `ImportReviewService` list/detail and accept/reject parsed snapshots with audit. There is no approved production Import / Export UI or Operational Mapping activation gate. |
