...
| Deterministic Console-Mobile trial bridge | Verified in repository | `packages/trial-model/src/bridge.ts` and the clients provide an optional linked-window channel with expected-window/origin validation, ephemeral session/request correlation, action acknowledgement and deduplication, heartbeat loss detection, and explicit local-only fallback. It transports synthetic in-memory state/actions only; it is not persistence, an API contract, or production sync. |
...
| Deterministic trial task assignments | Verified in repository | Trial mode applies Tier 1 tracking-owner changes and Tier 2 direct-report field delegation through the shared reducer. The reducer checks current tracking/field assignment before Tier 2/Tier 3 execution or task-record mutation, and the Mobile bridge rejects Tier 1-only Console actions. Tier 2/Tier 3 projections update immediately in the linked or standalone local state and reset deterministically. |
