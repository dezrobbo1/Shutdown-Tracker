...
Tier 1 authority is operational authority within the selected project. It does not allow Shutdown Tracker to calculate schedules, bypass immutable evidence or audit, or silently change a Microsoft Project master file.
...
Those business descriptions do not create another application role, and the product must not assume that every Tier 1 user personally maintains the Microsoft Project schedule. Any future external Project open, save, adoption, or merge/import workflow requires its own approved contract; the relevant schedule owner or Microsoft Project operator performs that business activity under project governance.
