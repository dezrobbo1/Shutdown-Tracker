...
pager.innerHTML =
  '<button type="button" id="task-prev">Previous</button><span id="task-page-info"></span><button type="button" id="task-next">Next</button>';
...
style.textContent = `
.task-browser .table-scroll{max-height:62vh;overflow:auto}
.task-detail{align-self:start}
.lab-pager{display:flex;gap:.6rem;align-items:center;justify-content:flex-end;margin:.5rem 0}
.task-context{display:block;font-size:.78rem;opacity:.72;margin-top:.15rem}
.task-browser{min-height:0}
.status-message.warning{border-left-color:#b36b00;background:#fff7e6}
`;
...
function formatPlan(task) {
  return `${task.start?.replace("T", " ") ?? "—"} → ${task.finish?.replace("T", " ") ?? "—"}`;
}
...
function touchedTaskUids() {
  return [...new Set(state.events.map((event) => String(event.taskUid)))];
}
...
function projectSeedTime(parsed) {
  for (const value of [parsed.project.currentDate, parsed.project.statusDate, parsed.leafTasks[0]?.start]) {
...
function renderProjectSummary() {
  ...
  appendDefinitionList(elements["project-summary"], [
    ...
    ["All timephased rows", source.parsed.timephasedData?.length ?? 0]
  ]);
...
function filteredTasks() {
  ...
  return state.source.parsed.leafTasks.filter(
    (task) =>
      !search ||
      [task.name, task.uid, task.id, task.wbs, task.outlineNumber, task.hierarchyPath]
        .filter(Boolean)
...
function renderTaskRows() {
  clearElement(elements["task-rows"]);
  if (!state.source) {
    return;
  }
  ...
  const visible = tasks.slice(state.taskPage * PAGE_SIZE, (state.taskPage + 1) * PAGE_SIZE);
  ...
  for (const task of visible) {
    ...
    if (task.hierarchyPath) {
      const context = document.createElement("span");
      context.className = "task-context";
      context.textContent = task.hierarchyPath;
      name.append(context);
    }
  ...
  pageInfo.textContent = tasks.length
    ? `${state.taskPage * PAGE_SIZE + 1}–${Math.min((state.taskPage + 1) * PAGE_SIZE, tasks.length)} of ${tasks.length}`
    : "0 tasks";
  prevButton.disabled = state.taskPage === 0;
  nextButton.disabled = state.taskPage >= pageCount - 1;
}
...
function renderTaskDetail() {
  ...
  appendDefinitionList(elements["selected-task-facts"], [
    ...
    ["Hierarchy", task.hierarchyPath || "Top-level leaf"],
    ...
    ["Task timephased rows", task.timephasedData?.length ?? 0],
...
function renderEvents() {
  ...
  const events = state.events.slice().sort((left, right) => (right.sequence ?? 0) - (left.sequence ?? 0));
  ...
  for (const event of events) {
    ...
    time.textContent = (event.effectiveProjectLocalTime ?? event.timestamp).replace("T", " ");
    time.title = `Recorded ${event.recordedAtUtc} · ${event.recordedTimeZone} · UTC offset ${event.recordedOffsetMinutes} min`;
...
function renderProfile() {
  ...
  elements["profile-warning"].className =
    `callout ${profile.classification === "diagnostic" ? "danger" : "info"}`;
}
...
function renderCandidateSummary() {
  ...
  appendDefinitionList(elements["candidate-summary"], [
    ...
    [
      "Source preserved",
      candidate.profile.id === "intent-only"
        ? "Byte-for-byte"
        : "Complete source with bounded task scalar edits"
    ]
  ]);
...
function render() {
  elements.workspace.hidden = !state.source;
...
function addEvent(type, options = {}) {
  ...
  state.events.push(
    createExecutionEvent({
      taskUid: task.uid,
      type,
      timestamp: options.timestamp ?? trialInstant(),
      recordedAt: new Date(),
...
async function loadSource(file) {
  ...
  setStatus(
    ...
    `${parsed.project.name}: ${parsed.tasks.length} tasks, ${parsed.leafTasks.length} executable leaves, ${parsed.assignments.length} assignments and ${parsed.timephasedData.length} timephased rows loaded.`,
    "success"
  );
  setStatus(elements["result-status"], "No Project result loaded.");
  render();
...
async function generateAndDownloadCandidate() {
  ...
  state.candidate = { ...generated, bytes, hash, parsed, fileName };
  state.result = null;
  elements["result-file"].value = "";
  setStatus(elements["result-status"], "No Project result loaded.");
  render();
...
function executionLogDocument() {
  ...
  const profile = getExportProfile(elements["export-profile"].value);
  const touched = touchedTaskUids();
  return {
    format: "shutdown-tracker-xml-roundtrip-lab/v3",
    ...
    result: state.result
      ? {
          ...
          classification: state.result.compatibility.classification,
          label: state.result.compatibility.label,
          identityMethod: state.result.compatibility.identityMethod,
          warnings: state.result.compatibility.warnings
        }
      : null,
    tasks: touched.map((uid) => {
...
function compatibilityStatus(fileName, hash, compatibility) {
  const taskSet = compatibility.taskSet;
  const base =
    `${fileName} loaded as ${compatibility.label}. ` +
    `Identity basis: ${compatibility.identityMethod}. ` +
    `Task set: candidate ${taskSet.candidateCount}, result ${taskSet.resultCount}. ` +
    `SHA-256 ${hash}.`;
  return compatibility.warnings.length > 0
    ? `${base} Warnings: ${compatibility.warnings.join(" ")}`
    : base;
}
...
async function loadResult(file) {
  ...
  const compatibility = classifyProjectResultCompatibility({
    candidate: state.candidate.parsed,
    result: parsed,
    touchedTaskUids: touchedTaskUids()
  });

  state.result = { fileName: file.name, ...decoded, parsed, hash, compatibility };
  setStatus(
    elements["result-status"],
    compatibilityStatus(file.name, hash, compatibility),
    compatibility.classification === "strict-result" ? "success" : "warning"
  );
  renderComparison();
}
...
elements["progress-shift-end"].addEventListener("click", () => {
  ...
  addEvent(EXECUTION_EVENT_TYPES.PROGRESS_TO_SHIFT_END, {
    timestamp: shiftEnd,
    expectedPercent
  });
});
...
elements["download-log"].addEventListener("click", () => {
  try {
    ...
    const base = state.source.fileName
      .replace(/\.mspdi\.xml$/i, "")
      .replace(/\.xml$/i, "");
    downloadText(
